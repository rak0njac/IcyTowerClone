...
EyeCandy::EyeCandy(float &posX, float &posY)
{
	step = 5;									//used for changing colors. cannot use static in changeColor() because of wierd bug (some stars are white)
	std::random_device rd;
	std::mt19937 rand(rd());
	std::uniform_int_distribution<> randPos(-3, 3);
	std::uniform_real_distribution<> randDir(-1, 1);
